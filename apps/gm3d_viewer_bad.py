...
class MeshViewer3D(OpenGLBaseWidget):
    ...
    def _framebuffer_size(self):
        """Get framebuffer size with proper Legacy/Modern handling"""
        if OPENGL_LEGACY:
            # QGLWidget: logical size IS physical size (no DPR multiplication)
            return int(self.width()), int(self.height())
        else:
            # QOpenGLWidget: need DPR for framebuffer actual size
            dpr = self.devicePixelRatioF()
            return int(self.width() * dpr), int(self.height() * dpr)
    
    def _set_full_viewport(self):
        """Set full viewport and projection consistently"""
        ...
        glu.gluPerspective(45.0, aspect, 0.1, 10000.0)  # Conservative clipping for large models
    ...
    def paintGL(self):
        ...
        # Reset viewport and projection consistently every frame
        self._set_full_viewport()
        
        # Debug log for intermittent issue tracking
        print("DPR:", self.devicePixelRatioF(), "widget wh:", self.width(), self.height())
        print("frame VP:", gl.glGetIntegerv(gl.GL_VIEWPORT))
        ...
        if self.wireframe_mode:
            ...
            gl.glDisable(gl.GL_LIGHTING)            # Wireframe must have lighting OFF
            gl.glDisable(gl.GL_CULL_FACE)           # No culling needed for lines
            gl.glColor3f(0.0, 1.0, 0.0)  # Green wireframe
        else:
            gl.glPolygonMode(gl.GL_FRONT_AND_BACK, gl.GL_FILL)
            gl.glDisable(gl.GL_CULL_FACE)           # Disable culling first to test visibility
            gl.glEnable(gl.GL_LIGHTING)
            gl.glEnable(gl.GL_LIGHT0)
        ...
        if self.faces and self.vertices:
            ...
            print("wire:", self.wireframe_mode,
                  "VP:", gl.glGetIntegerv(gl.GL_VIEWPORT),
                  "lighting:", bool(gl.glIsEnabled(gl.GL_LIGHTING)))
            print("#V:", len(self.vertices), "#F:", len(self.faces))
            
            # Set up lighting for solid mode
    ...
    def resizeGL(self, width, height):
        ...
        if not OPENGL_AVAILABLE:
            return
        # Use same function as paintGL for consistency
        self._set_full_viewport()
    ...
    def draw_coordinate_axes(self):
        ...
        # Save current viewport (contains FB pixels)
        prev_vp = gl.glGetIntegerv(gl.GL_VIEWPORT)
        
        # Set small viewport in bottom-left corner with proper Legacy/Modern handling  
        if OPENGL_LEGACY:
            pad, axes_size = 10, 100
        else:
            dpr = self.devicePixelRatioF()
            pad, axes_size = int(10 * dpr), int(100 * dpr)
        gl.glViewport(pad, pad, axes_size, axes_size)
        ...
        # Restore saved viewport (no DPR recalculation!)
        gl.glViewport(int(prev_vp[0]), int(prev_vp[1]), int(prev_vp[2]), int(prev_vp[3]))
        
        # Debug log for intermittent issue tracking
        print("after axes VP:", gl.glGetIntegerv(gl.GL_VIEWPORT))
