 ...
 const vector<WH_Polygon3D>& polygon_s)
{
  ...
  for (int iPoly = 0; iPoly < nPolys; iPoly++) {
    ...
    // Use more tolerant comparison for small-scale geometry
    bool normalMatches = WH_eq (plane.normal (), outsideNormal);
    if (!normalMatches) {
      // Try more tolerant comparison for precision-sensitive small geometry
      normalMatches = WH_eq2 (plane.normal (), outsideNormal);
      if (normalMatches) {
        cerr << "WARNING: Using relaxed precision for small-scale geometry normal comparison" << endl;
      } else {
        cerr << "ERROR: Plane normal mismatch even with relaxed precision" << endl;
        cerr << "   Calculated normal: (" << plane.normal().x << ", " << plane.normal().y << ", " << plane.normal().z << ")" << endl;
        cerr << "   Expected normal:   (" << outsideNormal.x << ", " << outsideNormal.y << ", " << outsideNormal.z << ")" << endl;
      }
    }
    WH_ASSERT(normalMatches);
