...
Windows is advcad's main execution environment (Akio, 2026-07-26) — do not treat the Windows-related files at the workspace root as stale or unreferenced just because no doc previously mentioned them; this was a documentation gap, not an abandoned experiment.

Cross-compiled from Linux via MinGW-w64, using the root-level `toolchain-mingw64.cmake`:
```bash
...
`advcad-windows-v0.13.0/` (and its `.zip`) is a prebuilt distribution snapshot from this process: `advcad.exe`, `INSTALL-WINDOWS.txt`, `README`, `copyright`. Both are committed to git as-is (binary + archive) — not regenerated by the build system, so they will drift from `WH/`/`command/` source unless manually rebuilt and replaced.
