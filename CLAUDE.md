...
### Windows Build (Primary Execution Environment)
Windows is advcad's main execution environment (Akio, 2026-07-26) — do not treat the Windows-related files as stale or unreferenced just because no doc previously mentioned them; this was a documentation gap, not an abandoned experiment. Windows-specific files live under `windows/` (consolidated 2026-07-27, previously scattered at the workspace root).

Cross-compiled from Linux via MinGW-w64, using `windows/toolchain-mingw64.cmake`:
```bash
# Requires the mingw-w64 cross-compiler (not installed on this dev machine as of 2026-07-26,
# so this exact invocation is untested this session — verify before relying on it):
mkdir build-windows && cd build-windows
cmake -DCMAKE_TOOLCHAIN_FILE=../windows/toolchain-mingw64.cmake ..
make -j4
# Produces a statically-linked command/advcad.exe
```

`windows/advcad-windows-v0.13.0/` (and its `.zip`) is a prebuilt distribution snapshot from this process: `advcad.exe`, `INSTALL-WINDOWS.txt`, `README`, `copyright`. Both are committed to git as-is (binary + archive) — not regenerated by the build system, so they will drift from `WH/`/`command/` source unless manually rebuilt and replaced.
